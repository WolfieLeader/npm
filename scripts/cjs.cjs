// CJS functional tests — encrypt/decrypt, hashing, compression, certs, Express integration.
const http = require("node:http");
const { createSecretKey, encrypt, decrypt, tryHash } = require("cipher-kit/node");
const { compress, decompress } = require("compress-kit");
const { generateCerts } = require("generate-certs");
const { getClientIp } = require("get-client-ip");
const { setCookie } = require("modern-cookies");
...
(async () => {
  ...
  console.log("cjs.cjs: all functional tests OK");
})();
